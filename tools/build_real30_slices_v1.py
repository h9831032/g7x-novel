...
# -*- coding: utf-8 -*-
"""
G7X REAL30 Slice Builder v1

Reads REAL30_REAL_A.txt (30 lines) and splits into:
- REAL30_DAY_S1.txt ~ S5.txt (5 slices, 6 lines each)
- REAL30_NIGHT_S1.txt ~ S5.txt (5 slices, 6 lines each)
...
SOURCE_FILE = GPTORDER_DIR / "REAL30_REAL_A.txt"

SLICE_SIZE = 6  # 6 missions per slice
EXPECTED_LINES = 30  # 5 slices * 6 lines
...
def main():
    print("[REAL30_SLICES] Starting slice generation...")
    ...
    # 6. Split into 5 slices (6 lines each)
    slices = []
    for i in range(5):
    ...
    for i, slice_lines in enumerate(slices):
        day_file = GPTORDER_DIR / f"REAL30_DAY_S{i + 1}.txt"
    ...
    for i, slice_lines in enumerate(slices):
        night_file = GPTORDER_DIR / f"REAL30_NIGHT_S{i + 1}.txt"
        with open(night_file, "w", encoding="utf-8") as f:
            f.write("\n".join(slice_lines) + "\n")
        print(f"[NIGHT] Created {night_file.name} ({len(slice_lines)} lines)")

    print("")
    print("[SUCCESS] All REAL30 slices created!")
    print(f"  DAY: REAL30_DAY_S1.txt ~ S5.txt")
    print(f"  NIGHT: REAL30_NIGHT_S1.txt ~ S5.txt")
    print(f"  Total: 10 files, {EXPECTED_LINES} missions")
