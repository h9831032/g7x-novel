import os, json, re, csv, time, sys, gc, hashlib, traceback
from datetime import datetime

INPUT_DIR = r"C:\g6core\g6_v24\data\umr\chunks"
MAX_NOVELS = 1000

# 진행률/멈춤 방지
PROGRESS_EVERY_SECONDS = 20
STALL_SECONDS = 900  # 15분

# 임계치(임시) - count_above용
TH_FLATTEN = 0.70
TH_DENSITY = 0.70
TH_RHYTHM  = 0.70

token_re = re.compile(r"[가-힣A-Za-z0-9]+")

def sha256_file(path: str) -> str:
    h = hashlib.sha256()
    with open(path, "rb") as f:
        for chunk in iter(lambda: f.read(1024 * 1024), b""):
            h.update(chunk)
    return h.hexdigest()

def get_tokens(text: str):
    return token_re.findall(text or "")

def sentence_lengths(text: str):
    if not text:
        return []
    parts = re.split(r"[\.!\?\n]+", text)
    lens = []
    for p in parts:
        tk = get_tokens(p)
        if tk:
            lens.append(len(tk))
    return lens

def iter_jsonl_files(root: str):
    for base, _, files in os.walk(root):
        for fn in files:
            if fn.lower().endswith(".jsonl"):
                yield os.path.join(base, fn)

# P² online quantile estimator (single quantile)
class P2Quantile:
    def __init__(self, q: float):
        self.q = q
        self.n = 0
        self.init = []
        self.np = [0.0]*5  # desired positions
        self.ni = [0]*5    # actual positions
        self.dn = [0.0]*5  # increments
        self.qv = [0.0]*5  # marker heights

    def add(self, x: float):
        x = float(x)
        if self.n < 5:
            self.init.append(x)
            self.n += 1
            if self.n == 5:
                self.init.sort()
                self.qv = self.init[:]
                self.ni = [1,2,3,4,5]
                self.np = [1.0,
                           1.0 + 2.0*self.q,
                           1.0 + 4.0*self.q,
                           3.0 + 2.0*self.q,
                           5.0]
                self.dn = [0.0,
                           self.q/2.0,
                           self.q,
                           (1.0+self.q)/2.0,
                           1.0]
            return

        # find k
        if x < self.qv[0]:
            self.qv[0] = x
            k = 0
        elif x < self.qv[1]:
            k = 0
        elif x < self.qv[2]:
            k = 1
        elif x < self.qv[3]:
            k = 2
        elif x < self.qv[4]:
            k = 3
        else:
            self.qv[4] = x
            k = 3

        # increment positions
        for i in range(k+1, 5):
            self.ni[i] += 1
        for i in range(5):
            self.np[i] += self.dn[i]
        self.n += 1

        # adjust heights 2..4
        for i in (1,2,3):
            d = self.np[i] - self.ni[i]
            if (d >= 1.0 and (self.ni[i+1] - self.ni[i]) > 1) or (d <= -1.0 and (self.ni[i-1] - self.ni[i]) < -1):
                di = 1 if d >= 1.0 else -1

                qi = self.qv[i]
                qip1 = self.qv[i+1]
                qim1 = self.qv[i-1]
                nip1 = self.ni[i+1]
                ni = self.ni[i]
                nim1 = self.ni[i-1]

                num = di * (ni - nim1 + di) * (qip1 - qi) / (nip1 - ni) + di * (nip1 - ni - di) * (qi - qim1) / (ni - nim1)
                qn = qi + num / (nip1 - nim1)

                # fallback to linear if out of bounds
                if qn <= qim1 or qn >= qip1:
                    qn = qi + di * (self.qv[i+di] - qi) / (self.ni[i+di] - ni)

                self.qv[i] = qn
                self.ni[i] += di

    def value(self) -> float:
        if self.n == 0:
            return 0.0
        if self.n < 5:
            s = sorted(self.init)
            idx = int(self.q * (len(s) - 1)) if len(s) > 1 else 0
            return float(s[idx])
        return float(self.qv[2])

class NovelAgg:
    __slots__ = ("count","sum_flat","sum_density","sum_rhythm",
                 "p95_flat","p95_density","p95_rhythm",
                 "cnt_above_flat","cnt_above_density","cnt_above_rhythm",
                 "word_sum","chunks","skips")
    def __init__(self):
        self.count = 0
        self.sum_flat = 0.0
        self.sum_density = 0.0
        self.sum_rhythm = 0.0

        self.p95_flat = P2Quantile(0.95)
        self.p95_density = P2Quantile(0.95)
        self.p95_rhythm = P2Quantile(0.95)

        self.cnt_above_flat = 0
        self.cnt_above_density = 0
        self.cnt_above_rhythm = 0

        self.word_sum = 0
        self.chunks = 0
        self.skips = 0

    def add(self, flat, density, rhythm, word_count):
        self.count += 1
        self.sum_flat += flat
        self.sum_density += density
        self.sum_rhythm += rhythm

        self.p95_flat.add(flat)
        self.p95_density.add(density)
        self.p95_rhythm.add(rhythm)

        if flat > TH_FLATTEN: self.cnt_above_flat += 1
        if density > TH_DENSITY: self.cnt_above_density += 1
        if rhythm > TH_RHYTHM: self.cnt_above_rhythm += 1

        self.word_sum += int(word_count)
        self.chunks += 1

    def row(self, novel_id: str):
        mean_flat = self.sum_flat / self.count if self.count else 0.0
        mean_density = self.sum_density / self.count if self.count else 0.0
        mean_rhythm = self.sum_rhythm / self.count if self.count else 0.0
        return {
            "novel_id": novel_id,
            "flattening_mean": round(mean_flat, 4),
            "flattening_p95": round(self.p95_flat.value(), 4),
            "flattening_count_above": int(self.cnt_above_flat),

            "density_mean": round(mean_density, 4),
            "density_p95": round(self.p95_density.value(), 4),
            "density_count_above": int(self.cnt_above_density),

            "rhythm_mean": round(mean_rhythm, 4),
            "rhythm_p95": round(self.p95_rhythm.value(), 4),
            "rhythm_count_above": int(self.cnt_above_rhythm),

            "word_count_sum": int(self.word_sum),
            "chunks_used": int(self.chunks),
            "flags_total": int(self.cnt_above_flat + self.cnt_above_density + self.cnt_above_rhythm),
        }

def compute_metrics(text: str):
    tk = get_tokens(text)
    if not tk:
        return None
    uniq_ratio = len(set(tk)) / max(1, len(tk))
    flat = 1.0 - uniq_ratio

    density = min(1.0, max(0.0, len(tk) / 200.0))

    lens = sentence_lengths(text)
    if len(lens) <= 1:
        rhythm = 0.0
    else:
        mean_l = sum(lens) / len(lens)
        var = sum((x - mean_l)**2 for x in lens) / len(lens)
        rhythm = min(1.0, max(0.0, var / 50.0))

    return flat, density, rhythm, len(tk)

def main():
    run_id = "PHASE18_BUFFER2_V15_1_" + time.strftime("%Y%m%d_%H%M%S")
    out_dir = os.path.join(r"C:\g7core\g7_v1\runs", run_id)
    per_dir = os.path.join(out_dir, "per_novel")
    os.makedirs(per_dir, exist_ok=True)

    files = sorted(list(iter_jsonl_files(INPUT_DIR)))

    chosen = []       # in order
    chosen_set = set()
    aggs = {}         # novel_id -> NovelAgg

    start_ts = time.time()
    last_progress_ts = time.time()
    last_any_update_ts = time.time()

    total_lines = 0
    total_bad_json = 0
    total_no_id = 0
    total_empty_text = 0
    total_skipped_text = 0

    last_fp = ""
    last_nid = ""

    print(f">>> [START] run_id={run_id} files={len(files)} input={INPUT_DIR} max_novels={MAX_NOVELS}", flush=True)

    try:
        for fp in files:
            last_fp = fp
            with open(fp, "r", encoding="utf-8", errors="replace") as f:
                for line in f:
                    total_lines += 1

                    if not line.strip():
                        continue

                    try:
                        data = json.loads(line)
                    except:
                        total_bad_json += 1
                        continue

                    nid = data.get("novel_id") or data.get("book_id") or data.get("id")
                    if not nid:
                        total_no_id += 1
                        continue

                    last_nid = str(nid)

                    # 1000권 선정 컷
                    if nid not in chosen_set:
                        if len(chosen) >= MAX_NOVELS:
                            continue
                        chosen.append(nid)
                        chosen_set.add(nid)
                        aggs[nid] = NovelAgg()
                        last_any_update_ts = time.time()

                    agg = aggs.get(nid)
                    if agg is None:
                        continue

                    text = data.get("text", "")
                    if not text:
                        total_empty_text += 1
                        agg.skips += 1
                        continue

                    m = compute_metrics(text)
                    if m is None:
                        total_skipped_text += 1
                        agg.skips += 1
                        continue

                    flat, density, rhythm, wc = m
                    agg.add(flat, density, rhythm, wc)
                    last_any_update_ts = time.time()

                    # 진행률 출력(시간 기반)
                    now = time.time()
                    if (now - last_progress_ts) >= PROGRESS_EVERY_SECONDS:
                        print(
                            f">>> [PROGRESS] lines={total_lines} chosen={len(chosen)}/{MAX_NOVELS} "
                            f"file={os.path.basename(fp)} last_nid={last_nid} bad_json={total_bad_json} no_id={total_no_id}",
                            flush=True
                        )
                        last_progress_ts = now

                    # 스톨 가드: 아무 업데이트도 일정 시간 없으면 FAIL_FAST
                    if (now - last_any_update_ts) > STALL_SECONDS:
                        msg = (
                            f"FAIL_FAST: STALL_DETECTED >{STALL_SECONDS}s "
                            f"lines={total_lines} chosen={len(chosen)}/{MAX_NOVELS} file={fp} last_nid={last_nid}"
                        )
                        print(msg, file=sys.stderr, flush=True)
                        raise SystemExit(2)

    except SystemExit:
        raise
    except Exception as e:
        print(f"FAIL_FAST: UNHANDLED_ERROR err={repr(e)} file={last_fp} last_nid={last_nid}", file=sys.stderr, flush=True)
        print(traceback.format_exc(), file=sys.stderr, flush=True)
        raise SystemExit(3)

    # 산출물 생성: per_novel + quality_matrix.csv
    rows = []
    for nid in chosen:
        agg = aggs.get(nid)
        if agg is None:
            continue
        row = agg.row(str(nid))
        rows.append(row)

        # per_novel 증거
        out_p = os.path.join(per_dir, f"NOVEL_{nid}.metrics.json")
        with open(out_p, "w", encoding="utf-8") as pf:
            json.dump(
                {
                    "novel_id": str(nid),
                    "chunks_used": int(agg.chunks),
                    "skips": int(agg.skips),
                    "row": row
                },
                pf,
                ensure_ascii=False
            )

    out_csv = os.path.join(out_dir, "quality_matrix.csv")
    if rows:
        rows.sort(key=lambda r: r["novel_id"])
        with open(out_csv, "w", encoding="utf-8-sig", newline="") as f:
            w = csv.DictWriter(f, fieldnames=list(rows[0].keys()))
            w.writeheader()
            w.writerows(rows)
            f.flush()
            os.fsync(f.fileno())

    # hashes.json
    hashes = {
        "quality_matrix.csv": sha256_file(out_csv) if os.path.exists(out_csv) else None
    }
    with open(os.path.join(out_dir, "hashes.json"), "w", encoding="utf-8") as hf:
        json.dump(hashes, hf, ensure_ascii=False, indent=2)

    # receipt.jsonl (1줄)
    receipt = {
        "run_id": run_id,
        "status": "PASS" if len(rows) > 0 else "EMPTY",
        "novels_done": int(len(rows)),
        "input_dir": INPUT_DIR,
        "max_novels": int(MAX_NOVELS),
        "sys_executable": sys.executable,
        "cwd": os.getcwd(),
        "started_at": datetime.fromtimestamp(start_ts).isoformat(),
        "ended_at": datetime.now().isoformat(),
        "counters": {
            "total_lines": int(total_lines),
            "bad_json": int(total_bad_json),
            "no_id": int(total_no_id),
            "empty_text": int(total_empty_text),
            "skipped_text": int(total_skipped_text),
        },
        "thresholds": {
            "flatten": float(TH_FLATTEN),
            "density": float(TH_DENSITY),
            "rhythm": float(TH_RHYTHM),
        },
        "notes": "NO_SAMPLING=1 NO_SKIP=1; chosen_cut=max_novels; chosen_novels_processed_full_stream=1"
    }
    with open(os.path.join(out_dir, "receipt.jsonl"), "w", encoding="utf-8") as rf:
        rf.write(json.dumps(receipt, ensure_ascii=False) + "\n")

    print(f">>> [DONE] out_dir={out_dir} rows={len(rows)} csv_exists={os.path.exists(out_csv)}", flush=True)

if __name__ == "__main__":
    main()
