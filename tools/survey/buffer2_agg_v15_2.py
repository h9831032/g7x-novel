import os, json, re, csv, time, sys, gc, hashlib, traceback
...
STALL_SECONDS_ANY_UPDATE = 900   # 15분 동안 어떤 업데이트도 없으면 FAIL
STALL_SECONDS_NO_COMPLETE = 1200 # 20분 동안 "완성(60청크)" 소설이 0개면 FAIL
...
# P² online quantile estimator (single quantile)
class P2Quantile:
...
def main():
    run_id = "PHASE18_BUFFER2_V15_2_" + time.strftime("%Y%m%d_%H%M%S")
    ...
    print(f">>> [START] run_id={run_id} files={len(files)} input={INPUT_DIR} max_novels={MAX_NOVELS} chunks_per_novel={CHUNKS_PER_NOVEL}", flush=True)
    ...
    try:
        for fp in files:
            ...
            with open(fp, "r", encoding="utf-8", errors="replace") as f:
                for line in f:
                    ...
                    # 이미 60청크 완성한 소설은 더 먹여도 되지만, 속도 위해 스킵(완성 후 추가 집계는 선택)
                    if agg.complete:
                        continue
                    ...
                    before_complete = agg.complete
                    agg.add(flat, density, rhythm, wc)
                    last_any_update_ts = time.time()

                    if (not before_complete) and agg.complete:
                        complete_count += 1
                        last_complete_ts = time.time()
                    ...
                    if (now - last_progress_ts) >= PROGRESS_EVERY_SECONDS:
                        print(
                            f">>> [PROGRESS] lines={total_lines} chosen={len(chosen)}/{MAX_NOVELS} complete={complete_count}/{MAX_NOVELS} "
                            f"file={os.path.basename(fp)} last_nid={last_nid} bad_json={total_bad_json} no_id={total_no_id}",
                            flush=True
                        )
                        last_progress_ts = now

                    if (now - last_any_update_ts) > STALL_SECONDS_ANY_UPDATE:
                        msg = f"FAIL_FAST: STALL_ANY_UPDATE >{STALL_SECONDS_ANY_UPDATE}s lines={total_lines} chosen={len(chosen)} complete={complete_count} file={fp} last_nid={last_nid}"
                        print(msg, file=sys.stderr, flush=True)
                        raise SystemExit(2)

                    if (now - last_complete_ts) > STALL_SECONDS_NO_COMPLETE:
                        msg = f"FAIL_FAST: STALL_NO_COMPLETE >{STALL_SECONDS_NO_COMPLETE}s lines={total_lines} chosen={len(chosen)} complete={complete_count} file={fp} last_nid={last_nid}"
                        print(msg, file=sys.stderr, flush=True)
                        raise SystemExit(2)
        ...
        print(f"FAIL_FAST: UNHANDLED_ERROR err={repr(e)} file={last_fp} last_nid={last_nid}", file=sys.stderr, flush=True)
    ...
    for nid in chosen:
        ...
        out_p = os.path.join(per_dir, f"NOVEL_{nid}.metrics.json")
            ...
            json.dump(
                {
                    ...
                    "chunks_per_novel_required": int(CHUNKS_PER_NOVEL),
                    "row": row
                },
                pf,
                ensure_ascii=False
    ...
    out_ins_csv = os.path.join(out_dir, "insufficient_novels.csv")

    if complete_rows:
        complete_rows.sort(key=lambda r: r["novel_id"])
        ...
        # 완성 0이면 빈 파일이라도 만들어 “없어서 못 찾는” 사고 방지
        with open(out_csv, "w", encoding="utf-8-sig", newline="") as f:
            f.write("novel_id,insufficient_chunks\n")

    if insufficient_rows:
        insufficient_rows.sort(key=lambda r: r["novel_id"])
    ...
    hashes = {
        ...
        "insufficient_novels.csv": sha256_file(out_ins_csv) if os.path.exists(out_ins_csv) else None,
    }
    ...
    receipt = {
        ...
        "novels_chosen": int(len(chosen)),
        "novels_complete": int(len(complete_rows)),
        "novels_insufficient": int(len(insufficient_rows)),
        "input_dir": INPUT_DIR,
        "max_novels": int(MAX_NOVELS),
        "chunks_per_novel_required": int(CHUNKS_PER_NOVEL),
        ...
        "notes": "NO_SAMPLING=1 NO_SKIP=1; complete_requires_chunks=60; complete_rows_only_in_main_csv=1"
    }
    with open(os.path.join(out_dir, "receipt.jsonl"), "w", encoding="utf-8") as rf:
        rf.write(json.dumps(receipt, ensure_ascii=False) + "\n")

    print(f">>> [DONE] out_dir={out_dir} chosen={len(chosen)} complete={len(complete_rows)} insufficient={len(insufficient_rows)} csv_exists={os.path.exists(out_csv)}", flush=True)
